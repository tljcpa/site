...
先说清楚这是个什么项目，免得显得我在吹。这是我自己做的一个内容合规质检模型——微调 Qwen2.5-14B-Instruct，覆盖四个平台（小红书、抖音、视频号、公众号），输入一段文案加一批平台规则，输出是否违规、违规原文、并**精确溯源到具体哪一条规则**。我给自己定了条硬线：溯源率 90%。

整个项目跑了好几个星期、五六次微调实验。但越往后我越意识到，真正难的不是把溯源率做上去，而是**搞清楚这个数字到底该怎么算**——因为同一个模型，换个口径，溯源率能从 29% 一直滑到 96%。这篇就讲这条主线。它也是我做过最诚实的一段工程：在"挑个好看的口径交差"和"把口径说清楚"之间，我反复选了后者。
