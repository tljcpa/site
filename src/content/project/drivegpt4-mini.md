...
1. **数据生成**：驾驶场景图用 PIL 合成（480×320，含道路、红绿灯、行人、前车、十字路口等元素，固定随机种子保证可复现），再用 Azure GPT-4.1-mini 的视觉接口对每张图自动生成【场景描述+决策+原因】标注，零人工。生成 50 条，按 0.85 切成 42 训练 + 8 测试。
2. **微调**：用 ms-swift 的 `swift sft`，QLoRA（4bit NF4 + 双重量化，LoRA rank=16 / alpha=32），5 epoch、有效 batch=16、lr=1e-4 cosine。可训练参数 18.5M / 2227M，只占 **0.83%**。
3. **部署**：`swift export` 合并 LoRA 成独立模型，可进一步 INT8 量化（约 4.5GB → 2.3GB）。
